...
def map_cta_labels(preds, test, text_to_label):
    ...
    for j, table_preds in enumerate(preds):
        ...
        for c, pred in enumerate(col_preds[:table_number]):
            i+=1
            ...
            if pred in text_to_label:
                ...
                if fin == "":
                    ...
                    if fin2=="":
                        # print(f"For test example {i} out of label space prediction: {pred}")
                        predictions.append('-')
                        oov_indices.append(i-1)
                        oov_table_indices.append(j)
                        oov_tablecolumn_indices.append(c)
                        oov.append(pred)
                        num +=1
                    else:
                        predictions.append(text_to_label[fin2])
        ...
        if len(col_preds) < table_number:
            for m in range(0, table_number-len(col_preds)):
                ...
                oov_table_indices.append(j)
                oov_tablecolumn_indices.append(c+m)
                oov.append(pred)
                num +=1
                i+=1
    return predictions, num, oov_indices, oov, oov_table_indices, oov_tablecolumn_indices
